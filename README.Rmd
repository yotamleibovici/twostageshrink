...
The \code{reproduce()} function uses the data in the \code{data/} directory,
which is generated with scripts that can be found in the \code{data-raw/}
...
help you are \code{simulate()}, \code{perform()}, \code{evaluate()} and
\code{measure()}, in this order. For the shrinkage figures, the relevant
functions to generate data and manipulate it similarly to the shown in the
thesis are \code{shrink_simulate()}, \code{shrink_estimate()},
\code{shrink_evaluate()} and \code{shrink_reproduce()}, in this order.
